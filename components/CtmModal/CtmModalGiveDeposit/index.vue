  ...
  >
    <div class="deposit__content сontent">
      <div class="content__step">
        <div
          v-for="(item, i) in modalSteps"
          :key="i"
          class="content__panel"
          :class="{'content__panel_active': step === item.step}"
...
export default {
  ...
  computed: {
    modalSteps() {
      return [
        {
          step: 1, selector: 'PREVIOUS-STEP', action: this.previousStep, text: this.$t('modals.walletAddress'),
        },
        {
          step: 2, selector: 'NEXT-STEP', action: this.nextStep, text: this.$t('meta.bankCard'),
        },
      ];
    },
  },
